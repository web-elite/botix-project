...
class StartCommand
{
    ...
    {
        if (this_id_is_admin((int) $bot->chatId())) {
    ...
    protected function adminKeyboard(): ReplyKeyboardMarkup
    {
        return ReplyKeyboardMarkup::make()
            ->addRow(
                KeyboardButton::make('افزودن کاربر 👤'),
                KeyboardButton::make('غیرفعالسازی درگاه')
            )
            ->addRow(
                KeyboardButton::make('حذف کاربران تستی 🗑️'),
            )
            ->addRow(
